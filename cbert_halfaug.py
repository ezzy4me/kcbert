# make average vector of [0], [1] segment vector
# take that average vector into input embedding.
...
def convert_ids_to_str(ids, tokenizer):
    ...
    tokens = []
    for token_id in ids:
        # logger.info('%s',token_id)
        # logger.info('%s',tokenizer)
        token = tokenizer._convert_id_to_token(int(token_id))
        tokens.append(token)
...
def main():
    ...
    for e in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        cbert_name = "{}/BertForMaskedLM_{}_epoch_{}".format(task_name.lower(), task_name.lower(), 10)
        model = load_model(cbert_name)
        # averge of toekn_type_embeddings
        ...
        model.bert.embeddings.token_type_embeddings = token_type_embeddings        
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            for ids, idx, preds, seg in zip(init_ids, masked_idx, predictions, segment_ids):
                ...
                for pred in preds:
                    ...
                    tsv_writer.writerow([new_str.decode('utf-8'), seg[0].item()])
                    # tsv_writer.writerow([new_str.encode("utf-8"), seg[0].item()])
            torch.cuda.empty_cache()
