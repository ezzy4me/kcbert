...
#         if name in ['stsa.binary', 'mpqa', 'rt-polarity', 'subj', 'nsmc']:
#             return ["0", "1"]
#         elif name in ['korean-hate-speech-detection']:
#             return ["0", "1", "2"]
...
def convert_examples_to_features(examples, label_list, labels, max_seq_length, tokenizer):
    """Loads a data file into a list of 'InputBatch's."""
    
    label_map = {}
    # logger.info("%s",label_list)
    ...
    for (example, label) in zip(examples, labels):
        ...
        tokens_a = tokenizer._tokenize(example)    
        # tokens_a = tokenizer._tokenize(example.text_a)
        tokens_label = label_map[label]
...
def construct_train_dataloader(train_examples, label_list, labels, max_seq_length, train_batch_size, num_train_epochs, tokenizer, device):
    ...
    train_features = convert_examples_to_features(
        train_examples, label_list, labels, max_seq_length, tokenizer)
