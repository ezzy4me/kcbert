...
from transformers import BertTokenizerFast, BertTokenizer, BertModel, BertForMaskedLM, AdamW, get_linear_schedule_with_warmup #WarmupLinearSchedule
...
def main():
    ...
    if task_name == 'stsa.fine': #stanford-sentiment-dataset
    ...
    if task_name == 'stsa.binary': #stanford-sentiment-dataset
        model.bert.embeddings.token_type_embeddings = torch.nn.Embedding(2, 768)
        model.bert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=0.02)
    if task_name == 'nsmc': #naver_movie_commnet_korean
        model.bert.embeddings.token_type_embeddings = torch.nn.Embedding(2, 768)
        model.bert.embeddings.token_type_embeddings.weight.data.normal_(mean=0.0, std=0.02)    
